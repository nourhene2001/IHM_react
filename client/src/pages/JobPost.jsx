...
function PostJob() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => navigate('/jobs/my-jobs'), 2000);
    } catch (error) {
      console.error('Error posting job:', error);
      setErrors({
        general: error.response?.data?.message || 'An error occurred while posting the job. Please try again.'
      });
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center container py-8">
      <motion.div
      ...
      >
        ...
        {success ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="toast success">
            Job posted successfully! Redirecting to your jobs...
          </motion.div>
        ) : (
          <div className="card max-w-3xl mx-auto">
            {errors.general && (
              <div className="toast error mb-6">{errors.general}</div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="form-group">
                  ...
                  />
                  {errors.company && <p className="text-error-500 text-sm mt-1">{errors.company}</p>}
                </div>
              </div>
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="List the skills, experience, education..."
                ></textarea>
                {errors.requirements && <p className="text-error-500 text-sm mt-1">{errors.requirements}</p>}
              </div>

             

              <div className="flex justify-end pt-4">
                <button type="button" className="btn btn-neutral mr-3" onClick={() => navigate('/jobs')}>
                  Cancel
                </button>
                <motion.button
